...
async function initializeSampleData(supabase) {
  ...
  try {
    ...
    if (
      productsError &&
      !productsError.message.includes('relation "products" does not exist')
    ) {
      throw productsError;
    }
    ...
    if (!existingProducts || existingProducts.length === 0) {
      ...
      if (insertError) {
        console.log(
          "⚠️  Could not insert sample products:",
          insertError.message,
        );
      } else {
    ...
    if (!existingSettings || existingSettings.length === 0) {
      ...
      if (settingsError) {
        console.log(
          "⚠️  Could not insert global settings:",
  ...
  try {
    ...
    for (const bucket of buckets) {
      ...
      if (!existingBucket) {
        ...
        if (error) {
          console.log(
            `⚠️  Could not create bucket ${bucket.name}:`,
            error.message,
          );
        } else {
