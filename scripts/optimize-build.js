...
try {
  ...
  if (existsSync(distPath)) {
    const assets = readdirSync(join(distPath, "assets")).filter((f) =>
      f.endsWith(".js"),
    );
    ...
    if (totalSize > 2 * 1024 * 1024) {
      // 2MB warning
      console.log(
        "⚠️ Warning: Total bundle size is large. Consider code splitting.",
      );
    } else {
...
try {
  ...
  const buildFiles = execSync(
    'find dist/ -name "*.js" -o -name "*.html" -o -name "*.css"',
    { encoding: "utf8" },
  );
