...
import { 
  Search, Filter, Grid, List, MoreVertical, Download, Eye, Edit,
  Trash2, Share, Clock, CheckCircle, XCircle, AlertTriangle,
  Tag, User, Calendar, FileText, Video, Image, File,
  GitBranch, MessageSquare, ThumbsUp, ThumbsDown, Archive
} from 'lucide-react';
...
export default function EnhancedFileManager({
  ...
  const loadAssets = async () => {
    try {
      setLoading(true);
      setError(null);
      const searchResults = await enhancedFileStorageService.searchFiles(filters);
  ...
  const loadVersionHistory = async (asset: FileAsset) => {
    try {
      const history = await enhancedFileStorageService.getVersionHistory(asset.id);
  ...
  const loadChangelog = async (asset: FileAsset) => {
    try {
      const changes = await enhancedFileStorageService.getAssetChangelog(asset.id);
        ...
        <TabsList>
          ...
          {showApprovalWorkflow && canApprove && (
            <TabsTrigger value="pending">
              Pending Approval ({assets.filter(a => a.status === 'pending_approval').length})
            </TabsTrigger>
          )}
        </TabsList>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Select value={filters.bucket || 'all'} onValueChange={(value) => 
                  updateFilters({ bucket: value === 'all' ? undefined : value as BucketName })
                }>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="charge-source-user-files">👤 User Files</SelectItem>
                    <SelectItem value="charge-source-documents">📄 Documents</SelectItem>
                    <SelectItem value="charge-source-videos">🎥 Videos</SelectItem>
                  </SelectContent>
                </Select>

                <Select value={filters.status || 'all'} onValueChange={(value) => 
                  updateFilters({ status: value === 'all' ? undefined : value as AssetStatus })
                }>
                  ...
                  {filteredAssets.map((asset) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              ...
                              {allowVersionControl && (
                                <>
                                  <DropdownMenuItem onClick={() => handleCreateNewVersion(asset)}>
                                    <GitBranch className="h-4 w-4 mr-2" />
                                    New Version
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => loadVersionHistory(asset)}>
                                    <Clock className="h-4 w-4 mr-2" />
                                    Version History
                                  </DropdownMenuItem>
                                </>
                              )}
                              <DropdownMenuItem onClick={() => loadChangelog(asset)}>
                                <MessageSquare className="h-4 w-4 mr-2" />
                                Changelog
                              </DropdownMenuItem>
                              {showApprovalWorkflow && canApprove && asset.status === 'pending_approval' && (
                                <>
                                  <DropdownMenuSeparator />
                        ...
                        <h4 className="font-medium text-sm mb-2 truncate" title={asset.title || asset.fileName}>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-muted-foreground">v{asset.versionNumber}</span>
                          </div>
        ...
        {showApprovalWorkflow && canApprove && (
          <TabsContent value="pending">
            <Card>
              ...
              <CardContent>
                {assets.filter(a => a.status === 'pending_approval').map(asset => (
                  <div key={asset.id} className="border rounded-lg p-4 mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium">{asset.title || asset.fileName}</h4>
                    ...
                    <div className="flex items-center space-x-4 text-xs text-muted-foreground">
                      <span>{formatFileSize(asset.fileSize)}</span>
                      <span>{formatDate(asset.createdAt)}</span>
                      <span>📂 {asset.bucketName.replace('charge-source-', '')}</span>
                    </div>
                  </div>
                ))}
              </CardContent>
      ...
      <Dialog open={showApprovalDialog} onOpenChange={setShowApprovalDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <p>
              Are you sure you want to {approvalAction} "{selectedAsset?.title || selectedAsset?.fileName}"?
            </p>
