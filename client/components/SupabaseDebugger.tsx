  ...
  const runDebugTests = async () => {
    ...
    try {
      ...
      if (debugResults.connectionTest.success) {
        try {
          const testResults = {};
          const buckets = ['charge-source-user-files', 'charge-source-documents', 'charge-source-videos'];
      ...
      <CardContent>
        {!results ? (
          <Alert>
            ...
            <AlertDescription>
              Click "Run Debug Tests" to diagnose Supabase connection and bucket access issues.
            </AlertDescription>
          </Alert>
        ) : (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="bg-gray-100 p-3 rounded text-sm font-mono">
                ...
                <div>Development: {results.environment.isDev ? 'Yes' : 'No'}</div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <Badge variant={results.supabaseConfig.supabaseUrl ? "default" : "destructive"}>
                    URL: {results.supabaseConfig.supabaseUrl ? 'Set' : 'Missing'}
                  </Badge>
                  <Badge variant={results.supabaseConfig.hasAnonKey ? "default" : "destructive"}>
                    Anon Key: {results.supabaseConfig.hasAnonKey ? `${results.supabaseConfig.anonKeyLength} chars` : 'Missing'}
                  </Badge>
                </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <Badge variant={results.connectionTest.success ? "default" : "destructive"}>
                ...
                {results.connectionTest.buckets && results.connectionTest.buckets.length > 0 && (
                  <div className="text-sm">
                    <strong>Found buckets:</strong> {results.connectionTest.buckets.join(', ')}
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <Badge variant={results.authTest.hasUser ? "default" : "secondary"}>
                  {results.authTest.hasUser ? 'Authenticated' : 'Not authenticated'}
                </Badge>
            ...
            {/* Bucket Access Test */}
            {results.bucketTest && Object.keys(results.bucketTest).length > 0 && (
              <div>
                <h4 className="font-medium mb-2">Bucket Access Test</h4>
                <div className="space-y-2">
                  {Object.entries(results.bucketTest).map(([bucket, test]: [string, any]) => (
