...
export default function AuthTest() {
  ...
  const quickTestLogin = async () => {
    ...
    try {
      ...
      if (data.user) {
        alert(
          "✅ Test login successful! You can now run the authentication test.",
        );
      ...
      alert(
        `❌ Test login failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      ...
      <CardContent className="space-y-4">
        ...
        {authState && (
          <div className="space-y-3">
              ...
              <AlertDescription>
                ...
                {authState.getUser?.success ? (
                  <div className="text-sm mt-1">
                    User: {authState.getUser.email} (ID:{" "}
                    {authState.getUser.userId})
                  </div>
                ) : (
            ...
            {/* getSession test */}
            <Alert
              variant={authState.getSession?.success ? "default" : "default"}
            >
              <AlertDescription>
                <div className="font-semibold">
                  getSession():{" "}
                ...
                {authState.getSession?.success ? (
                  <div className="text-sm mt-1">
                    {authState.getSession.session ? (
                      <>
                        Session: {authState.getSession.session.email} (Expires:{" "}
                        {authState.getSession.session.expiresAt})
                      </>
                    ) : (
            ...
            <Alert
              variant={authState.buckets?.success ? "default" : "destructive"}
            >
              <AlertDescription>
                <div className="font-semibold">
                  Storage Buckets:{" "}
                  {authState.buckets?.success ? "✅ Success" : "❌ Failed"}
                </div>
                {authState.buckets?.success ? (
                  <div className="text-sm mt-1">
                    Buckets:{" "}
                    {authState.buckets.buckets.length > 0
                      ? authState.buckets.buckets.join(", ")
                      : "No buckets found"}
                  </div>
                ) : (
            ...
            {/* Database test */}
            <Alert
              variant={authState.database?.success ? "default" : "destructive"}
            >
              <AlertDescription>
                <div className="font-semibold">
                  Database Query:{" "}
                  {authState.database?.success ? "✅ Success" : "❌ Failed"}
                </div>
