...
class SupabaseProductService {
  ...
  async addProduct(product: Omit<ProductCatalogueItem, 'id' | 'createdAt' | 'updatedAt'>): Promise<ProductCatalogueItem> {
    const { data, error } = await supabase
      .from('products')
      .insert({
        sku: product.sku,
  ...
  async updateProduct(productId: string, updates: Partial<ProductCatalogueItem>): Promise<ProductCatalogueItem | null> {
    const { data, error } = await supabase
      .from('products')
      .update({
  ...
  async getCategories(): Promise<Array<{ id: string; name: string }>> {
    ...
    return categories.map(cat => ({
      id: cat,
      name: cat.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())
    }))
  }

  // Get subcategories for a category
  async getSubcategories(category: string): Promise<Array<{ id: string; name: string }>> {
    const { data, error } = await supabase
      .from('products')
      .select('subcategory')
    ...
    return subcategories.map(subcat => ({
      id: subcat,
      name: subcat.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())
    }))
  }
  ...
  async getPopularProducts(limit: number = 10): Promise<ProductCatalogueItem[]> {
  ...
  async getLowStockProducts(threshold: number = 10): Promise<ProductCatalogueItem[]> {
    const { data, error } = await supabase
      .from('products')
      .select('*')
      .eq('is_active', true)
    ...
    return (data || []).filter(product => 
      (product.inventory?.available || 0) <= threshold
    ).sort((a, b) => 
      (a.inventory?.available || 0) - (b.inventory?.available || 0)
    )
  }
  ...
  async bulkImportProducts(products: Omit<ProductCatalogueItem, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<ProductCatalogueItem[]> {
    ...
    // Log activity
    await this.logActivity('products_bulk_imported', 'product', '', { count: products.length })

    return data || []
  }

  // Search products with advanced filters
  async searchProducts(query: string, filters?: ProductFilter): Promise<ProductCatalogueItem[]> {
    const searchFilter = { ...filters, searchTerm: query }
    return this.getProducts(searchFilter)
  }

  // Helper method to log activity
  private async logActivity(action: string, resourceType: string, resourceId: string, details?: any) {
