...
import { Quote, QuoteLineItem, QuoteTemplate, ProductCatalogueItem } from '../quoteTypes'
...
class SupabaseQuoteService {
  ...
  // Add line item
  async addLineItem(quoteId: string, lineItem: Omit<QuoteLineItem, 'id' | 'totalPrice'>): Promise<Quote | null> {
    ...
    const { data: calculation } = await supabase.functions.invoke('calculate-quote-totals', {
      body: {
  ...
  async updateLineItem(
    ...
    const lineItemIndex = quote.line_items.findIndex((item: any) => item.id === lineItemId)
    if (lineItemIndex === -1) return null

    const updatedLineItems = [...quote.line_items]
    updatedLineItems[lineItemIndex] = { ...updatedLineItems[lineItemIndex], ...updates }
  ...
  // Remove line item
  async removeLineItem(quoteId: string, lineItemId: string): Promise<Quote | null> {
    const quote = await this.getQuote(quoteId)
    if (!quote) return null

    const updatedLineItems = quote.line_items.filter((item: any) => item.id !== lineItemId)
  ...
  async addProductToQuote(quoteId: string, productId: string, quantity: number = 1): Promise<Quote | null> {
    // Get product details
  ...
  // Create template
  async createTemplate(template: Omit<QuoteTemplate, 'id' | 'createdAt' | 'usageCount'>): Promise<QuoteTemplate> {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error('User not authenticated')
