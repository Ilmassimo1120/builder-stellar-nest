...
export class PipedriveCustomerProvider implements CustomerProvider {
  ...
  async authenticate(config: Partial<CRMConfig>): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/persons?api_token=${config.apiKey}&limit=1`, {
        headers: {
  ...
  async getCustomers(limit = 100, offset = 0): Promise<Customer[]> {
    ...
    try {
      ...
      return (data.data || []).map((person: any) => this.mapPipedrivePersonToCustomer(person));
    } catch (error) {
  ...
  async getCustomer(id: string): Promise<Customer | null> {
    ...
    try {
      const pipedriveId = id.startsWith('pipedrive_') ? id.replace('pipedrive_', '') : id;
  ...
  async createCustomer(customerData: Omit<Customer, 'id' | 'createdAt' | 'updatedAt' | 'source'>): Promise<Customer> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/persons?api_token=${this.config!.apiKey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async updateCustomer(id: string, updates: Partial<Customer>): Promise<Customer> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/persons/${pipedriveId}?api_token=${this.config!.apiKey}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async getCustomerDeals(customerId: string): Promise<CustomerDeal[]> {
    ...
    try {
      const pipedriveId = customerId.startsWith('pipedrive_') ? customerId.replace('pipedrive_', '') : customerId;
      ...
      return (data.data || []).map((deal: any) => this.mapPipedriveDealToCustomerDeal(deal, customerId));
    } catch (error) {
  ...
  async createDeal(dealData: Omit<CustomerDeal, 'id' | 'createdAt' | 'updatedAt' | 'source'>): Promise<CustomerDeal> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/deals?api_token=${this.config!.apiKey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      return this.mapPipedriveDealToCustomerDeal(data.data, dealData.customerId);
    } catch (error) {
  ...
  async updateDeal(id: string, updates: Partial<CustomerDeal>): Promise<CustomerDeal> {
    ...
    try {
      const pipedriveId = id.startsWith('pipedrive_deal_') ? id.replace('pipedrive_deal_', '') : id;
      ...
      if (updates.probability !== undefined) pipedriveData.probability = updates.probability;
      if (updates.expectedCloseDate) pipedriveData.expected_close_date = updates.expectedCloseDate;
      if (updates.stage) pipedriveData.stage_id = this.mapStageToStageId(updates.stage);

      const response = await fetch(`${this.baseUrl}/deals/${pipedriveId}?api_token=${this.config!.apiKey}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      return this.mapPipedriveDealToCustomerDeal(data.data, updates.customerId!);
    } catch (error) {
  ...
  async getCustomerContacts(customerId: string): Promise<CustomerContact[]> {
    ...
    try {
      ...
      return (data.data || []).map((activity: any) => this.mapPipedriveActivityToContact(activity, customerId));
    } catch (error) {
  ...
  async addContact(contact: Omit<CustomerContact, 'id'>): Promise<CustomerContact> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/activities?api_token=${this.config!.apiKey}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  private mapCustomerToPipedrive(customer: Partial<Customer>): any {
    ...
    if (customer.email) {
      pipedriveData.email = [{ value: customer.email, primary: true, label: 'work' }];
    }
    if (customer.phone) {
      pipedriveData.phone = [{ value: customer.phone, primary: true, label: 'work' }];
    }
  ...
  private mapPipedriveDealToCustomerDeal(deal: any, customerId: string): CustomerDeal {
    return {
  ...
  private mapPipedriveActivityToContact(activity: any, customerId: string): CustomerContact {
    return {
  ...
  private mapContactTypeToPipedriveType(type: 'email' | 'phone' | 'meeting' | 'note'): string {
  ...
  private mapPipedriveTypeToContactType(type: string): 'email' | 'phone' | 'meeting' | 'note' {
