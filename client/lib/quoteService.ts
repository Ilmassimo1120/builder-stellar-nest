...
const DEFAULT_MARGIN_SETTINGS: MarginSettings = {
  ...
  maximumDiscount: 25,
  volumeDiscounts: [
    { minimumQuantity: 5, discountPercentage: 5, applicableCategories: ['chargers'] },
    { minimumQuantity: 10, discountPercentage: 10, applicableCategories: ['chargers'] },
    { minimumQuantity: 20, discountPercentage: 15, applicableCategories: ['chargers'] },
  ],
};
...
class QuoteService {
  ...
  private loadFromStorage(): void {
    try {
      ...
      if (marginsData) {
        this.marginSettings = { ...DEFAULT_MARGIN_SETTINGS, ...JSON.parse(marginsData) };
      }
    } catch (error) {
  ...
  private saveToStorage(): void {
    try {
      ...
      localStorage.setItem('chargeSourceQuoteTemplates', JSON.stringify(this.templates));
      localStorage.setItem('chargeSourceMarginSettings', JSON.stringify(this.marginSettings));
    } catch (error) {
  ...
  addLineItem(quoteId: string, lineItem: Omit<QuoteLineItem, 'id' | 'totalPrice'>): Quote | null {
    ...
    const newLineItem: QuoteLineItem = {
      ...lineItem,
      id: `line-${Date.now()}`,
      totalPrice: this.calculateLineItemTotal(lineItem.quantity, lineItem.unitPrice, lineItem.markup),
    };

    quote.lineItems.push(newLineItem);
    quote.totals = calculateQuoteTotals(quote.lineItems, quote.totals.discount, quote.totals.discountType);
    
    return this.updateQuote(quote);
  }

  updateLineItem(quoteId: string, lineItemId: string, updates: Partial<QuoteLineItem>): Quote | null {
    const quote = this.getQuote(quoteId);
    if (!quote) return null;
    ...
    if (updates.quantity !== undefined || updates.unitPrice !== undefined || updates.markup !== undefined) {
  ...
  private calculateLineItemTotal(quantity: number, unitPrice: number, markup: number): number {
    return quantity * unitPrice * (1 + markup / 100);
  }

  calculateMarginForCategory(category: string): number {
    return this.marginSettings.categoryMarkups[category] || this.marginSettings.defaultMarkup;
  }
    ...
    return lineItems.map(item => {
      ...
      if (applicableDiscount) {
        ...
        return {
          ...
          totalPrice: this.calculateLineItemTotal(item.quantity, discountedPrice, item.markup),
        };
      }
  ...
  // Template operations
  createTemplate(template: Omit<QuoteTemplate, 'id' | 'createdAt' | 'usageCount'>): QuoteTemplate {
  ...
  applyTemplate(quote: Quote, template: QuoteTemplate): Quote {
    const lineItems = template.lineItems.map(item => ({
      ...
      totalPrice: this.calculateLineItemTotal(item.quantity, item.unitPrice, item.markup),
    }));
  ...
  private generateLineItemsFromProject(quote: Quote, projectData: ProjectIntegration): void {
    const chargerSelection = projectData.chargerSelection;
    
    if (chargerSelection.chargingType && chargerSelection.powerRating && chargerSelection.numberOfChargers) {
      ...
      const installationLineItem: Omit<QuoteLineItem, 'id' | 'totalPrice'> = {
        ...
        description: 'Complete installation including electrical work, mounting, and commissioning',
  ...
  private getMockCatalogueItems(category?: string): ProductCatalogueItem[] {
    ...
    return category ? mockItems.filter(item => item.category === category) : mockItems;
  }

  addProductToQuote(quoteId: string, productId: string, quantity: number = 1): Quote | null {
    const product = this.getProductCatalogueItems().find(p => p.id === productId);
  ...
  getQuoteAnalytics(): QuoteAnalytics {
    ...
    const totalValue = this.quotes.reduce((sum, quote) => sum + quote.totals.total, 0);
    ...
    const statusBreakdown = this.quotes.reduce((acc, quote) => {
      ...
      const defaultTemplates = [
        {
          ...
          description: 'Standard package for residential single or dual AC charger installation',
          ...
          lineItems: [
            {
              ...
              name: '7kW AC Charging Station',
              description: 'Wall-mounted AC charging station with Type 2 connector',
              category: 'chargers',
              quantity: 1,
              unitPrice: 2400,
              cost: 1680,
            ...
            },
            {
              ...
              description: 'Professional installation including electrical work and commissioning',
              category: 'installation',
              quantity: 1,
              unitPrice: 1500,
              cost: 900,
          ...
          settings: {
            ...
            notes: 'Installation includes all necessary electrical work and permits.',
        ...
        },
        {
          ...
          description: 'Complete commercial DC fast charging solution with multiple units',
          category: 'commercial',
          isDefault: true,
          lineItems: [
            {
              type: 'charger' as const,
              name: '50kW DC Fast Charging Station',
              description: 'Commercial DC fast charger with CCS2 and CHAdeMO connectors',
              category: 'chargers',
              quantity: 4,
              unitPrice: 65000,
              cost: 45500,
            ...
            },
            {
              ...
              description: 'Complete installation including site preparation, electrical work, and commissioning',
              category: 'installation',
              quantity: 1,
              unitPrice: 45000,
              cost: 27000,
            ...
            },
            {
              ...
              name: 'Weather Protection Canopy',
              description: 'Protective canopy structure for outdoor installation',
              category: 'accessories',
              quantity: 2,
              unitPrice: 8500,
              cost: 5950,
          ...
          settings: {
            validityDays: 60,
            terms: 'Payment terms: 30% deposit, 40% on delivery, 30% on completion.',
            notes: 'Project includes all necessary permits, grid connection coordination, and compliance certifications.',
