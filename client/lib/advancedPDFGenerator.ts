...
class AdvancedPDFGenerator {
  ...
  async generateQuotePDF(
    ...
    try {
      ...
      // Save the PDF
      pdf.save(`quote-${quote.quoteNumber}-${quote.clientInfo.company?.replace(/[^a-zA-Z0-9]/g, '') || 'client'}.pdf`);
    } catch (error) {
  ...
  private async addContentToPDF(
    ...
    // Client Information
    yPosition = this.addClientInfo(pdf, quote, leftMargin, yPosition, pageWidth);

    // Project Information
    if (quote.title || quote.description) {
      yPosition = this.addProjectInfo(pdf, quote, leftMargin, yPosition, pageWidth);
    }

    // Line Items
    if (quote.lineItems.length > 0) {
      yPosition = await this.addLineItems(pdf, quote, leftMargin, yPosition, pageWidth, options);
    }
  ...
  private addHeader(pdf: jsPDF, quote: Quote, leftMargin: number, yPosition: number, pageWidth: number): number {
    ...
    pdf.text('ChargeSource', leftMargin + 25, yPosition + 10, { align: 'center' });
    ...
    pdf.text(`Quote #${quote.quoteNumber}`, rightX, yPosition + 12, { align: 'right' });
    pdf.text(`Date: ${new Date().toLocaleDateString()}`, rightX, yPosition + 18, { align: 'right' });
    pdf.text(`Valid until: ${new Date(quote.validUntil).toLocaleDateString()}`, rightX, yPosition + 24, { align: 'right' });
    ...
    // Contact information
    pdf.setFontSize(8);
    pdf.text('Email: hello@chargesource.com.au | Phone: 1300 CHARGE | Web: www.chargesource.com.au', leftMargin, yPosition + 25);
  ...
  private addClientInfo(pdf: jsPDF, quote: Quote, leftMargin: number, yPosition: number, pageWidth: number): number {
    pdf.setTextColor(0, 0, 0);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
  ...
  private addProjectInfo(pdf: jsPDF, quote: Quote, leftMargin: number, yPosition: number, pageWidth: number): number {
    pdf.setTextColor(0, 0, 0);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
  ...
  private async addLineItems(
    ...
    headers.forEach((header, index) => {
      ...
      const textX = index === 0 ? currentX + 2 : 
                   index === 1 ? currentX + colWidths[index] / 2 : 
    ...
    for (const item of quote.lineItems) {
      ...
      pdf.text(item.quantity.toString(), currentX + colWidths[1] / 2, yPosition, { align: 'center' });

      currentX += colWidths[1];
      pdf.text(`$${item.unitPrice.toLocaleString()}`, currentX + colWidths[2] - 2, yPosition, { align: 'right' });

      currentX += colWidths[2];
      pdf.text(`$${item.totalPrice.toLocaleString()}`, currentX + colWidths[3] - 2, yPosition, { align: 'right' });
  ...
  private addTotals(pdf: jsPDF, quote: Quote, leftMargin: number, yPosition: number, pageWidth: number): number {
    ...
    pdf.text(`$${quote.totals.subtotal.toLocaleString()}`, totalsX + totalsWidth, yPosition, { align: 'right' });
    yPosition += 6;
    ...
    if (quote.totals.discount > 0) {
      ...
      pdf.text(`-$${quote.totals.discount.toLocaleString()}`, totalsX + totalsWidth, yPosition, { align: 'right' });
      yPosition += 6;
      pdf.setTextColor(0, 0, 0);
    }
    ...
    pdf.text(`$${quote.totals.gst.toLocaleString()}`, totalsX + totalsWidth, yPosition, { align: 'right' });
    yPosition += 6;
    ...
    pdf.text(`$${quote.totals.total.toLocaleString()}`, totalsX + totalsWidth, yPosition, { align: 'right' });

    return yPosition + 15;
  }

  private addTerms(pdf: jsPDF, quote: Quote, leftMargin: number, yPosition: number, pageWidth: number): number {
    // Check if we need a new page
    ...
    const termsData = [
      ...
      { label: 'Quote Validity', value: `${quote.settings.validityDays} days from date of issue` }
    ];
    ...
    if (quote.settings.terms) {
      ...
      yPosition += 5;
      pdf.setFont('helvetica', 'normal');
      const termsLines = pdf.splitTextToSize(quote.settings.terms, pageWidth - 40);
    ...
    if (quote.settings.notes) {
      ...
      yPosition += 5;
      pdf.setFont('helvetica', 'normal');
      const notesLines = pdf.splitTextToSize(quote.settings.notes, pageWidth - 40);
  ...
  private addFooter(pdf: jsPDF, quote: Quote, leftMargin: number, pageHeight: number, pageWidth: number): void {
    ...
    pdf.text('This quote was generated by ChargeSource Smart Quoting Engine', pageWidth / 2, footerY + 4, { align: 'center' });
    pdf.text('For questions about this quote, please contact us at hello@chargesource.com.au', pageWidth / 2, footerY + 8, { align: 'center' });
  }
