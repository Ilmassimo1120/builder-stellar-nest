...
const bucketInfo: Record<BucketName, { 
  label: string; 
...
export default function EnhancedFileStorage() {
  ...
  const getStorageUsagePercentage = (used: number, bucket?: BucketName) => {
    ...
    const totalLimit = bucket ? limits[bucket] : Object.values(limits).reduce((a, b) => a + b, 0);
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Enhanced File Storage</h1>
          <p className="text-muted-foreground">
            Manage files with advanced metadata, version control, and approval workflows
          </p>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {getStorageUsagePercentage(storageStats.totalSize).toFixed(1)}% of total capacity
                </p>
              </CardContent>
            </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {Object.entries(bucketInfo).map(([bucketName, info]) => {
                  ...
                  const stats = storageStats.bucketBreakdown[bucket] || { files: 0, size: 0 };
                    ...
                    <Card key={bucket} className="relative overflow-hidden">
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          ...
                          <Progress 
                            value={getStorageUsagePercentage(stats.size, bucket)} 
                            className="mt-2"
                          />
                          <p className="text-xs text-muted-foreground">
                            {getStorageUsagePercentage(stats.size, bucket).toFixed(1)}% of bucket limit
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  {Object.entries(storageStats.categoryBreakdown)
                    ...
                    <div key={category} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium">{stats.files} files</div>
                      ...
                      .map(([status, count]) => {
                        ...
                        return (
                          <div key={status} className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              <Icon className={`w-4 h-4 ${colors[status as keyof typeof colors]}`} />
                              <span className="text-sm font-medium capitalize">
                                {status.replace('_', ' ')}
                              </span>
                            </div>
