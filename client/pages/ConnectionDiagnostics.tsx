...
import {
  connectionFixService,
  type FixResult,
} from "@/lib/services/connectionFixService";
...
export default function ConnectionDiagnostics() {
  ...
  const runComprehensiveDiagnostics = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes("Failed to fetch")) {
          newResults.push({
            ...
            details:
              "Cannot reach Supabase servers. Check your internet connection and CORS settings.",
            timing,
          });
        } else {
          newResults.push({
            ...
            details:
              "Connection established but auth failed. This might be expected.",
            timing,
          });
        }
      } else {
      ...
      newResults.push({
        ...
        details:
          "Network error occurred. Check your internet connection and firewall settings.",
      });
    }
  ...
  const getRecommendations = () => {
    ...
    const supabaseConnectionIssues = failedTests.filter(
      (r) =>
    ...
    const bucketIssues = results.find(
      (r) => r.test === "Storage Buckets" && r.status !== "success",
    );
    if (bucketIssues) {
      recommendations.push({
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Comprehensive test of Supabase and Netlify connections to identify
              issues
            </CardDescription>
          </CardHeader>
